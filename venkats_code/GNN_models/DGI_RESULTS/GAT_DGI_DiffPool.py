...
class DiffPoolReadout(nn.Module):
    def __init__(self, feat_dim, num_nodes=100):
        super().__init__()
        self.gnn_embed = GATv2Conv(feat_dim, feat_dim, heads=1)
        self.assign_mlp = nn.Sequential(
            nn.Linear(feat_dim, feat_dim),
            nn.ReLU(),
            nn.Linear(feat_dim, num_nodes)
        )

    def forward(self, x, edge_index, batch):
        x_embed = F.relu(self.gnn_embed(x, edge_index))
        s = F.softmax(self.assign_mlp(x_embed), dim=-1)
        adj = to_dense_adj(edge_index, batch=batch)
        x_dense = to_dense_batch(x_embed, batch)[0]
...
def train_model(patient_graphs, filtered_graphs, in_channels, hidden_channels, out_channels, num_heads, epochs=100, lr=5e-4):
    model = GAT_DGI(in_channels, hidden_channels, out_channels, num_heads).to(device)
    ...
    for epoch in range(epochs):
        ...
        for graph in filtered_graphs.values():
            ...
            graph_emb_exp = graph_emb.view(1, -1).expand(node_emb.size(0), -1)
        ...
        if avg_loss < best_loss:
            best_loss = avg_loss
            cnt_wait = 0
            torch.save(model.state_dict(), '/home/vascul/vsayyalasomayajula/my-rdisk/r-divb/venkat/Proteomics/PlasmaAAA/GNN_models/DGI_RESULTS/GATv2/DiffPool/best_model_gat_dgi_diffpool.pth')
        else:
            cnt_wait += 1
    ...
    model.load_state_dict(torch.load('/home/vascul/vsayyalasomayajula/my-rdisk/r-divb/venkat/Proteomics/PlasmaAAA/GNN_models/DGI_RESULTS/GATv2/DiffPool/best_model_gat_dgi_diffpool.pth'))
    model.eval()
    embeddings = []
    with torch.no_grad():
...
save_dir = '/home/vascul/vsayyalasomayajula/my-rdisk/r-divb/venkat/Proteomics/PlasmaAAA/GNN_models/DGI_RESULTS/GATv2/DiffPool/'
