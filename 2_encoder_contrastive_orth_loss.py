...
def training_loop(lamda_orth =0.1):
    ...
    for fold, (train_idx, val_idx) in enumerate(kf.split(patients)):
        ...
        for epoch in range(epochs):
            ...
            val_losses_diff.append(epoch_val_loss_diff)


            recall_5 = (true_ranks <= 5).float().mean()
            median_rank = np.median(true_ranks)
            print(
                f"Val Loss: {epoch_val_loss:.4f} | Val Loss Shared: {epoch_val_loss_shared:.4f} | Val Loss Orth: {epoch_val_loss_diff:.4f} |  retrieval accuracy embedding: {retrieval_accuracy} |  retrieval accuracy proj: {retrieval_accuracy_proj}"
            )
        ...
        plt.plot(
            epochs_range,
            train_losses_orth,
            label="Orthogonality Train loss"
        )
        ...
        plt.plot(
            epochs_range,
            val_losses_diff,
            label="Orthogonality Validation loss"
        )
        ...
        plt.savefig(
            f"training_plots/fold_{fold + 1}_retrieval_acc.png",
            dpi=300
        )
